...
1. **Scrape**: A workflow runs every 9 hours to fetch fresh data from all sources.
...
### Installation
```bash
git clone https://github.com/ericschiller/northseattlebeer.git
cd northseattlebeer
uv sync
```
...
- **Frontend**: Nuxt 3 app in `frontend/` with TRUCKS / EVENTS tab toggle, modern "North Seattle Beer" branding.
...
```
frontend/
├── app.vue                  # Root component with new visual theme and tab toggle
├── components/
│   ├── AppHeader.vue        # Centered "North Seattle Beer" title with Newsreader font
│   ├── AppFooter.vue        # Clean footer with maintainer credits
│   ├── DaySection.vue       # Day group with modern indented headers
│   └── TruckItem.vue        # Card with multicolored pills and venue links
├── public/
